(() => {
  ...
  const injectThemeToggle = () => {
    ...
    themeToggle.setAttribute('size', '1.5');
    const tryInject = () => {
      const widgets = document.querySelector('.widgets');
      if (widgets) {
        widgets.insertAdjacentElement('afterend', themeToggle);
        const css = document.createElement('style');
        css.textContent = 'theme-button{display:block;margin:10px auto 20px;text-align:center;width:100%;}';
        document.head.appendChild(css);
        return true;
      }
      return false;
    };
    if (!tryInject()) setTimeout(tryInject, 500);
  };
