(() => {
  const func = (root, initTheme, changeTheme) => {
    ...
    const detectCurrentTheme = () => {
      ...
      return 'light'; // 默认为浅色模式
    };

    // 初始化时检测当前主题模式
    const currentTheme = detectCurrentTheme();
    if (currentTheme === 'dark' && !isMoved) {
      // 如果当前是深色模式且组件尚未切换到深色状态，则切换
      components.onclick();
    } else if (currentTheme === 'light' && isMoved) {
      // 如果当前是浅色模式且组件处于深色状态，则切换回浅色
      components.onclick();
    }
  };
  ...
  class ThemeButton extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // 【调整大小位置】可以通过属性调整按钮大小，例如：size="2" 或 size="4"
      const size = +this.getAttribute("size") || 3;
      const shadow = this.attachShadow({ mode: "closed" });
      const container = document.createElement("div");
      container.setAttribute("class", "container");
      // 【调整大小位置】这里控制按钮大小，可根据需要修改
  ...
  // 【注入到 widgets 元素下方创建的新容器中】
  const injectThemeToggle = () => {
    // 创建 theme-button 元素
    const themeToggle = document.createElement('theme-button');
    themeToggle.setAttribute('size', '1.5');  // 设置大小

    // 尝试找到 widgets 元素并注入按钮到其下方
    const widgetsContainer = document.querySelector('.widgets');
    if (widgetsContainer) {
      // 直接在 widgets 元素后面插入
      widgetsContainer.insertAdjacentElement('afterend', themeToggle);
